...
2. Каков IP-адрес у сервера gaia.cs.umass.edu? Каковы номера портов для отправки и приема
   ...
   - 128.119.245.12
   - 53653 для отправителя, для принимающей стороны 80

на фото выше все видно
...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - 1910147177
   - по наличию выставленного флага SYN в TCP пакете
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - 1817846249
   - 1910147178
   - поле sequence number SYN пакета от клиента к серверу + 1
   - по установленным флагам SYN + ACK в TCP пакете
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - 1, 636, 2084, 3532, 4980, 6428 (это relative seq number)
   - 3.487144, 3.488146, 3.488152, 3.488153, 3.488154, 3.488156
   - 3.618486, 3.620329, 3.620329, 3.620332, 3.620334, 3.620335 (ищем ack = seq_num + data_len)
   - 0.131342, 0.132182, 0.132177, 0.132178, 0.132179, 0.132178
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - 131776 / 0.132039 = 998008 (bytes/s) = 998.008164 (kb/s)
   - window_size / rtt
