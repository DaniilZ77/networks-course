...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - английский и русский: ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7
   - Host: gaia.cs.umass.edu - хост сервера, Connection: keep-alive - переиспользование tcp соединения для отправки http запросов, User-Agent - информация откуда пользователь сделал запрос, Accept - принимаемые форматы (text/html), Accept-Encoding - алгоритмы сжатия, которые клиент может расшифровать
3. Какой IP-адрес вашего компьютера? Какой адрес сервера gaia.cs.umass.edu?
   - Мой ip-адрес 192.168.0.153
...
![Response](./images/response1.png "Response")
...
1. Проверьте содержимое первого HTTP-запроса GET. Видите ли вы строку «IF-MODIFIED-SINCE» в HTTP GET?
   - такой строки в первом запросе нету
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - вернул в виде text/html формата
   - во вкладке `Line based text data` можно увидеть текст
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - If-Modified-Since: Tue, 28 Oct 2025 05:59:01 GMT, текста явно нету в ответе второго запроса
4. Какой код состояния HTTP и фраза возвращаются сервером в ответ на этот второй запрос
   ...
   - вернулся код 304 Not Modified
   - сервер содержимое файла явно не вернул

![Request](./images/request21.png "Request")
![Response](./images/response21.png "Response")
![Request](./images/request22.png "Request")
![Response](./images/response22.png "Response")
...
1. Сколько сообщений HTTP GET отправил ваш браузер? Какой номер пакета в трассировке
   ...
   - два get запроса (еще один на иконку)
   - номер пакета 238
...
3. Сколько сегментов TCP, содержащих данные, потребовалось для передачи одного HTTP ответа?
   - потребовалось 5 сегмента (5 Reassembled TCP segments)
4. Есть ли в передаваемых данных какая-либо информация заголовка HTTP, связанная с
   сегментацией TCP?
   - нету, потому что клиент собирает эти заголовки на уровне tcp и http не знает об этом ничего

![Request](./images/request3.png "Request")
![Response](./images/response3.png "Response")
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - 3 get запроса
   - 128.119.245.12, 128.119.245.12, 2.56.99.24
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - браузер начал загружать изображения параллельно, это видно по времени начала обоих загрузок, обе временные метки произошли до обоих ответов от серверов

![Request](./images/request4.png "Request")
...
![Response](./images/response5.png "Response")
